...
def test_get_directions(dc):
    ...
    dirs = _get_directions(gdf.geometry.iloc[0], intersection.iloc[0])
    # The Collection 3 test scene clips the polygon at both its south and west
    # edges.
    assert dirs == {"South", "West"}
...
def test_south_overedge(dc):
    ...
    assert drill_result["water"].iloc[0] == 41
...
def test_north_overedge(dc):
    ...
    assert len(drill_result) == 1
    assert drill_result["water"].iloc[0] == 44
